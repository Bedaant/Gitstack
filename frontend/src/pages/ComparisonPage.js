      ...
      <main className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <SEO
            ...
            description={t1 && t2 ? `Compare ${t1} and ${t2}. Pricing, setup time, founder-friendliness, and self-hosting difficulty. Open source alternatives explained.` : "Compare any two tools side by side. Pricing, setup time, and founder-friendliness breakdown for open source vs SaaS."}
            path="/compare"
          />
          <div className="text-center mb-12">
            <motion.div 
              ...
              className="inline-flex items-center justify-center w-20 h-20 bg-background border-4 border-foreground neo-shadow-lg mb-6"
            >
              <Scale className="w-6 h-6 text-foreground" strokeWidth={2} />
            </motion.div>
          ...
          <div className="neo-card p-4 md:p-10 bg-background mb-12 border-4 border-foreground shadow-[8px_8px_0px_0px_hsl(var(--foreground))]">
            <div className="grid md:grid-cols-2 gap-8 items-center relative">
              <div className="flex flex-col gap-2">
                <label className="font-mono text-[10px] uppercase font-black text-muted-foreground tracking-widest">Contender A</label>
                <input
                  type="text"
                  value={t1}
              ...
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:flex z-10">
                <motion.div 
                  ...
                  className="bg-foreground text-background w-14 h-14 rounded-none border-4 border-background flex items-center justify-center font-black rotate-12 shadow-xl cursor-help group"
                >
              ...
              <div className="flex flex-col gap-2">
                <label className="font-mono text-[10px] uppercase font-black text-muted-foreground tracking-widest">Contender B</label>
                <input
                  type="text"
                  value={t2}
          ...
          {!comparison && !loading && (
            <motion.div 
              ...
              className="mb-12"
            >
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-4 font-black">Community Battles:</p>
              <div className="flex flex-wrap gap-3">
                {PRESETS.map((p, i) => (
                  <button
                    key={i}
                    onClick={() => handleCompare(p.t1, p.t2)}
                    className="px-6 py-3 border-4 border-foreground font-black text-sm hover:bg-foreground hover:text-background hover:-translate-y-1 transition-all bg-background shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-1 active:translate-y-1"
                  >
          ...
          {loading && (
            <div className="text-center py-24">
              <div className="spinner mx-auto mb-8 w-16 h-16 border-8 border-t-black" style={{ borderTopColor: 'hsl(var(--foreground))', borderRadius: 0 }}></div>
              <h2 className="text-3xl font-black uppercase italic tracking-tighter">Analyzing Market Cap & GFM...</h2>
              <p className="text-muted-foreground mt-2 font-mono text-sm uppercase tracking-widest">Querying global intelligence on setup times</p>
            </div>
          )}
          ...
          <AnimatePresence>
            {comparison && (
              <motion.div 
                ...
                className="neo-card p-6 md:p-12 bg-pastel-lavender border-4 border-black mb-20 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] text-black" 
                data-testid="comparison-result"
              >
                <div className="flex justify-between items-start mb-10 pb-6 border-b-4 border-foreground/10">
                  <div className="flex items-center gap-4">
                    <div className="bg-foreground text-background p-3 font-black uppercase text-xs">Battle Report</div>
                ...
                <div className="mt-12 pt-8 border-t-4 border-foreground/10 flex flex-wrap gap-4">
                  ...
                  <button className="neo-btn neo-btn-secondary px-8 py-4 font-bold border-foreground flex items-center gap-2" onClick={() => window.print()}>
                    Export PDF
                  </button>
                </div>
              </motion.div>
