...
const EXAMPLE_STACK = [
  ...
  { name: "Resend", role: "Email delivery", difficulty: "Beginner", color: "bg-blue-100" },
    ...
    <section className="py-16 px-4 bg-zinc-50 border-y-4 border-black">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-zinc-500 mb-2 font-black">What you actually get</p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="neo-card overflow-hidden border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="p-4 space-y-3">
              {EXAMPLE_STACK.map((t, i) => (
                <div key={t.name} className={`flex items-center gap-3 p-3 border-2 border-black ${t.color}`}>
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 border border-black ${t.difficulty === 'Beginner' ? 'bg-green-100' : 'bg-yellow-100'}`}>{t.difficulty}</span>
                </div>
              ))}
              <button
          ...
          {/* Example: Roast */}
          <div className="neo-card overflow-hidden border-4 border-red-600 shadow-[8px_8px_0px_0px_rgba(239,68,68,0.4)] bg-black text-white">
            ...
            <div className="p-4">
              <div className="text-sm leading-relaxed text-zinc-300 space-y-3">
                {EXAMPLE_ROAST.split('\n\n').map((block, i) => {
                  ...
                  return (
                    <div key={i}>
                      <p className="font-black text-white text-sm">{head.replace(/\*\*/g, '')}</p>
                      {rest.length > 0 && <p className="text-zinc-400 text-xs mt-1 leading-relaxed">{rest.join(' ')}</p>}
                    </div>
                  );
                })}
              </div>
              <button
                ...
                className="neo-btn bg-white text-black w-full py-3 font-black text-sm flex items-center justify-center gap-2 mt-4"
              >
          ...
          <div className="neo-card overflow-hidden border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="p-4">
              <div className="bg-pastel-mint border-2 border-black p-4 text-center mb-4">
                <p className="text-xs font-mono uppercase tracking-widest font-black text-zinc-600">Annual Savings</p>
                <p className="text-5xl font-black italic">$1,752</p>
                <p className="text-xs font-bold text-zinc-500 mt-1">Per year · Forever free</p>
              </div>
              <div className="space-y-2">
                {EXAMPLE_SAVINGS.map(s => (
                  <div key={s.paid} className="flex items-center justify-between border-b border-zinc-100 pb-2">
